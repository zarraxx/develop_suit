...
function(stage1_archive_stem input_path out_var)
...
function(stage1_normalize_host_arch input_arch out_var)
...
function(stage1_pick_single_file out_var description)
  stage1_try_pick_single_file(${out_var} "${description}" ${ARGN})
  if("${${out_var}}" STREQUAL "")
    message(FATAL_ERROR "Could not find ${description}. Looked for: ${ARGN}")
  endif()
endfunction()

function(stage1_try_pick_single_file out_var description)
  set(_matches "")
...
function(stage1_download_file_once output_path url description)
...
function(stage1_ensure_default_archive out_var description cache_dir archive_name archive_url)
  ...
  if(NOT EXISTS "${_archive_path}")
    if(NOT STAGE1_DOWNLOAD_MISSING)
      message(FATAL_ERROR
        ...
        "Set STAGE1_DOWNLOAD_MISSING=ON, place the archive in cache/, or pass an explicit path.")
    endif()
    stage1_download_file_once("${_archive_path}" "${archive_url}" "${description}")
  endif()
  set(${out_var} "${_archive_path}" PARENT_SCOPE)
endfunction()

function(stage1_default_llvm_download_info out_filename_var out_url_var)
  stage1_normalize_host_arch("${CMAKE_HOST_SYSTEM_PROCESSOR}" _host_arch)
  if(_host_arch STREQUAL "x86_64")
    set(_archive_name "compiler-llvm-18.1.8-linux-x86_64.tar.gz")
    set(_archive_url "${STAGE1_LLVM_X86_64_URL}")
  elseif(_host_arch STREQUAL "aarch64")
    set(_archive_name "compiler-llvm-18.1.8-linux-aarch64.tar.gz")
    set(_archive_url "${STAGE1_LLVM_AARCH64_URL}")
  else()
    message(FATAL_ERROR
      "Automatic LLVM host toolchain download is not configured for host arch "
      "${CMAKE_HOST_SYSTEM_PROCESSOR}. Set STAGE1_LLVM_ARCHIVE or STAGE1_CLANG_ROOT explicitly.")
  endif()
...
function(stage1_resolve_default_llvm_archive out_var cache_dir)
  stage1_normalize_host_arch("${CMAKE_HOST_SYSTEM_PROCESSOR}" _host_arch)
  file(GLOB _all_archives LIST_DIRECTORIES FALSE "${cache_dir}/compiler-llvm-*-linux-*.tar.gz")
  if(NOT _all_archives)
    stage1_default_llvm_download_info(_default_llvm_name _default_llvm_url)
    stage1_ensure_default_archive(
  ...
  if(_filtered_count GREATER 1)
    message(FATAL_ERROR
      ...
      "Please set STAGE1_LLVM_ARCHIVE explicitly.")
  endif()
  ...
  message(FATAL_ERROR
    ...
    "Set STAGE1_LLVM_ARCHIVE explicitly or enable automatic download.")
endfunction()

function(stage1_extract_archive_once archive_path destination_dir)
  if(NOT EXISTS "${archive_path}")
    message(FATAL_ERROR "Archive does not exist: ${archive_path}")
  endif()

  file(SHA256 "${archive_path}" _archive_hash)
  set(_stamp_path "${destination_dir}/.stage1-extract.sha256")
...
function(stage1_unwrap_single_subdir root_dir marker_relpath out_var)
...
function(stage1_default_host_triple out_var)
  stage1_normalize_host_arch("${CMAKE_HOST_SYSTEM_PROCESSOR}" _host_arch)
  set(_host_os "linux")
...
function(stage1_resolve_archive_source out_var description cache_dir extract_parent_dir marker_relpath)
  ...
  if("${_archive_path}" STREQUAL "")
    ...
    if(DEFINED ARG_GLOB_PATTERNS AND NOT "${ARG_GLOB_PATTERNS}" STREQUAL "")
      stage1_try_pick_single_file(_resolved_archive "${description} archive" ${ARG_GLOB_PATTERNS})
    endif()
  ...
  stage1_archive_stem("${_archive_path}" _archive_stem)
  set(_extract_dir "${extract_parent_dir}/${_archive_stem}")
  stage1_extract_archive_once("${_archive_path}" "${_extract_dir}")
  stage1_unwrap_single_subdir("${_extract_dir}" "${marker_relpath}" _source_root_dir)
