...
extract_archive_source() {
  ...
  local archive_name="$2"
  local marker_path="$3"
  local archive_marker="${source_dir}/.groonga-source-archive"
...
build_xxhash() {
  cmake_install xxhash "${DEP_SOURCE_DIR}/xxHash/cmake_unofficial" \
...
build_msgpackc() {
  cmake_install msgpack-c "${DEP_SOURCE_DIR}/msgpack-c" \
    -DBUILD_SHARED_LIBS=ON \
  ...
  if [[ -f "${SDK_PREFIX}/lib/pkgconfig/msgpack-c.pc" ]]; then
    ln -sf msgpack-c.pc "${SDK_PREFIX}/lib/pkgconfig/msgpack.pc"
  fi
}

build_groonga() {
  cmake_install groonga "${DEP_SOURCE_DIR}/groonga" \
    -DBUILD_SHARED_LIBS=ON \
    -DFETCHCONTENT_FULLY_DISCONNECTED=ON \
    -DGRN_ALLOW_WARNING=ON \
    -DGRN_WITH_APACHE_ARROW=OFF \
    -DGRN_WITH_ZLIB=no \
    -DGRN_WITH_ZSTD=no \
    -DGRN_WITH_SIMDJSON=no \
    -DGRN_WITH_RAPIDJSON=no \
    -DGRN_WITH_MECAB=no \
    -DGRN_WITH_KYTEA=no \
    -DGRN_WITH_LIBSTEMMER=no \
    -DGRN_WITH_ZEROMQ=no \
    -DGRN_WITH_LIBEVENT=no \
    -DGRN_WITH_MESSAGE_PACK=system \
    -DGRN_WITH_OPENZL=no \
    -DGRN_WITH_XXHASH=system \
    -DGRN_WITH_LIBEDIT=no \
    -DGRN_WITH_LZ4=no \
    -DGRN_WITH_ROARING_BITMAPS=no \
    -DGRN_WITH_BLOSC=no \
    -DGRN_WITH_XSIMD=no \
...
validate_groonga() {
  [[ -f "${SDK_PREFIX}/include/xxhash.h" ]] || die "missing xxHash headers"
  [[ -f "${SDK_PREFIX}/include/msgpack.h" ]] || die "missing msgpack-c headers"
  [[ -f "${SDK_PREFIX}/include/groonga/groonga.h" ]] || die "missing Groonga headers"
  [[ -f "${SDK_PREFIX}/lib/pkgconfig/groonga.pc" ]] || die "missing Groonga pkg-config file"
  [[ -d "${SDK_PREFIX}/lib/cmake/Groonga" ]] || die "missing Groonga CMake package"

  if [[ "$TARGET_KIND" == "mingw" ]]; then
    find "${SDK_PREFIX}" \( -type f -o -type l \) \( -name 'libgroonga*.dll' -o -name 'libgroonga*.dll.a' \) | grep -q . || \
      die "missing Groonga DLL/import library"
    return 0
  fi

  find "${SDK_PREFIX}/lib" \( -type f -o -type l \) -name 'libgroonga.so*' | grep -q . || \
    die "missing Groonga shared library"
}
...
XXHASH_VERSION="${XXHASH_VERSION:-0.8.3}"
MSGPACKC_VERSION="${MSGPACKC_VERSION:-6.1.0}"
GROONGA_VERSION="${GROONGA_VERSION:-16.0.5}"
JOBS="${JOBS:-4}"
SDK_PREFIX="${SDK_PREFIX:-/opt/groonga-${GROONGA_VERSION}-${TARGET_TRIPLE}}"
...
[[ -d "$SDK_PREFIX" ]] || die "missing Groonga package prefix: ${SDK_PREFIX}"
...
if [[ "$CONFIGURE_BUILD_TRIPLE" == "$CONFIGURE_HOST_TRIPLE" ]]; then
  CONFIGURE_BUILD_TRIPLE="${ARCH}-groongabuild-linux-gnu"
fi
...
DEP_SOURCE_DIR="${BUILD_DIR}/src/groonga"
...
COMMON_CFLAGS="${COMMON_CFLAGS:-} -I${SDK_PREFIX}/include"
COMMON_CXXFLAGS="${COMMON_CXXFLAGS:-} -I${SDK_PREFIX}/include"
...
XXHASH_ARCHIVE="xxHash-${XXHASH_VERSION}.tar.gz"
MSGPACKC_ARCHIVE="msgpack-c-${MSGPACKC_VERSION}.tar.gz"
GROONGA_ARCHIVE="groonga-${GROONGA_VERSION}.tar.gz"

download_archive "https://github.com/Cyan4973/xxHash/archive/refs/tags/v${XXHASH_VERSION}.tar.gz" "$XXHASH_ARCHIVE"
download_archive "https://github.com/msgpack/msgpack-c/releases/download/c-${MSGPACKC_VERSION}/${MSGPACKC_ARCHIVE}" "$MSGPACKC_ARCHIVE"
download_archive "https://packages.groonga.org/source/groonga/${GROONGA_ARCHIVE}" "$GROONGA_ARCHIVE"

extract_archive_source "${DEP_SOURCE_DIR}/xxHash" "$XXHASH_ARCHIVE" "cmake_unofficial/CMakeLists.txt"
extract_archive_source "${DEP_SOURCE_DIR}/msgpack-c" "$MSGPACKC_ARCHIVE" "CMakeLists.txt"
extract_archive_source "${DEP_SOURCE_DIR}/groonga" "$GROONGA_ARCHIVE" "CMakeLists.txt"

log "Installing Groonga dependencies into ${SDK_PREFIX}"
build_xxhash
build_msgpackc
build_groonga
copy_cxx_runtime_libraries
...
render_template "${TEMPLATE_DIR}/README.groonga.in" "${SDK_PREFIX}/README.groonga" \
  "TARGET_TRIPLE=${TARGET_TRIPLE}" \
  "TARGET_KIND=${TARGET_KIND}" \
  "XXHASH_VERSION=${XXHASH_VERSION}" \
  "MSGPACKC_VERSION=${MSGPACKC_VERSION}" \
  "GROONGA_VERSION=${GROONGA_VERSION}"

log "Groonga package ready: ${SDK_PREFIX}"
