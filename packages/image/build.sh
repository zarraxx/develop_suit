...
usage() {
  cat <<'EOF'
Usage:
  ./packages/image/build.sh --target=<target> [options]
...
Options:
  --target=<target>                  Image dependency target, see list above
...
copy_or_extract_base_prefix() {
  ...
  local dir_path="$3"
  local package_dir="postgresql_dependencies-${PACKAGE_TRIPLE}"
  local release_package_dir="postgresql_dependencies-18-${PACKAGE_TRIPLE}"
  ...
  if [[ -n "$dir_path" ]]; then
    ...
    if [[ -d "${tmp_extract}/${release_package_dir}" ]]; then
      extracted_dir="${tmp_extract}/${release_package_dir}"
    elif [[ -d "${tmp_extract}/${package_dir}" ]]; then
      extracted_dir="${tmp_extract}/${package_dir}"
    elif [[ -f "${tmp_extract}/README.postgresql-dependencies" ]]; then
...
resolve_target "$TARGET" "image dependency target"
...
if [[ -z "$PACKAGE_NAME" ]]; then
  PACKAGE_NAME="image-${PACKAGE_TRIPLE}"
fi
...
if [[ "$CLEAN" -eq 1 ]]; then
  echo "-- cleaning image target: ${TARGET_TRIPLE}"
...
echo "-- image dependency archive ready: ${ARCHIVE_PATH}"
