...
POSTGRESQL_ENABLE_LLVM_JIT="${POSTGRESQL_ENABLE_LLVM_JIT:-1}"
...
if [[ "$POSTGRESQL_ENABLE_LLVM_JIT" == "1" && "$TARGET_KIND" == "linux" && -z "$POSTGRESQL_LLVM_ROOT" && "$ARCH" == "x86_64" ]]; then
  POSTGRESQL_LLVM_ROOT="$LLVM_ROOT"
fi

if [[ "$POSTGRESQL_ENABLE_LLVM_JIT" != "1" ]]; then
