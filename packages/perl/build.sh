...
usage() {
  cat <<'EOF'
Usage:
  ./packages/perl/build.sh --target=<target> [options]
...
Options:
  --target=<target>                  Perl target, see list above
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --perl-archive)
      ...
      PERL_ARCHIVE="$1"
      ;;
    --perl-target-runner=*) PERL_TARGET_RUNNER="${1#*=}" ;;
    --perl-target-runner)
      shift
...
resolve_target "$TARGET" "Perl target"
...
if [[ -n "$PERL_ARCHIVE" ]]; then
  [[ -f "$PERL_ARCHIVE" ]] || die "Perl source archive not found: ${PERL_ARCHIVE}"
...
[[ -f "${MOUNT_ROOT}/container_perl.sh" ]] || die "missing container script: ${MOUNT_ROOT}/container_perl.sh"

make_host_writable "$PACKAGE_ROOT"
mkdir -p "$CACHE_DIR" "$BUILD_DIR" "$OUT_BASE" "$DIST_DIR"

if [[ "$CLEAN" -eq 1 ]]; then
  echo "-- cleaning Perl target: ${TARGET_TRIPLE}"
...
container_args=(
  ...
  -e "PERL_TARGET_RUNNER=${PERL_TARGET_RUNNER}"
  -e "JOBS=${JOBS}"
...
echo "-- Perl archive ready: ${ARCHIVE_PATH}"
