...
usage() {
  cat <<'EOF'
Usage:
  ./packages/tcl/build.sh --target=<target> [options]
...
Options:
  --target=<target>             Tcl target, see list above
  --arch=<target>               Alias for --target
  --tcl-version=<ver>           Tcl version (default: 8.6.18)
  ...
  --tcl-archive=<tar>           Use a local Tcl source archive
...
find_local_python_deps_archive() {
  ...
  for archive_name in \
      ...
      "python_dependencies-${PACKAGE_TRIPLE}.tar.xz"; do
...
copy_or_extract_base_prefix() {
  ...
  if [[ -n "$dir_path" ]]; then
    ...
    if [[ -z "$extracted_dir" ]]; then
      if [[ -f "${tmp_extract}/README.python-dependencies" ]]; then
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --target|--arch)
      ...
      ;;
    target=*|arch=*) TARGET="${1#*=}" ;;
    --tcl-version=*) TCL_VERSION="${1#*=}" ;;
    --tcl-version)
      shift
      [[ $# -gt 0 ]] || die "--tcl-version requires a value"
      TCL_VERSION="$1"
      ;;
    ...
    --python-deps-dir|--dependency-dir)
      ...
      PYTHON_DEPS_DIR="$1"
      ;;
    --tcl-archive=*) TCL_ARCHIVE="${1#*=}" ;;
    --tcl-archive)
      shift
      [[ $# -gt 0 ]] || die "--tcl-archive requires a value"
      TCL_ARCHIVE="$1"
      ;;
...
resolve_target "$TARGET" "Tcl target"
...
if [[ -z "$PACKAGE_NAME" ]]; then
  PACKAGE_NAME="tcl-${TCL_VERSION}-${PACKAGE_TRIPLE}"
fi
...
if [[ -n "$TCL_ARCHIVE" ]]; then
  [[ -f "$TCL_ARCHIVE" ]] || die "Tcl source archive not found: ${TCL_ARCHIVE}"
  TCL_ARCHIVE="$(cd "$(dirname "$TCL_ARCHIVE")" && pwd)/$(basename "$TCL_ARCHIVE")"
fi
...
[[ -f "${MOUNT_ROOT}/container_tcl.sh" ]] || die "missing container script: ${MOUNT_ROOT}/container_tcl.sh"

make_host_writable "$PACKAGE_ROOT"
mkdir -p "$CACHE_DIR" "$BUILD_DIR" "$OUT_BASE" "$DIST_DIR"

if [[ "$CLEAN" -eq 1 ]]; then
  echo "-- cleaning Tcl target: ${TARGET_TRIPLE}"
...
docker_args=(
  ...
  -e "TCL_VERSION=${TCL_VERSION}"
  -e "JOBS=${JOBS}"
  -e "SDK_PREFIX=/opt/${PACKAGE_NAME}"
)
if [[ -n "$TCL_ARCHIVE" ]]; then
  docker_args+=(
    -v "${TCL_ARCHIVE}:/work/input/tcl${TCL_VERSION}-src.tar.gz:ro"
    -e "TCL_ARCHIVE=/work/input/tcl${TCL_VERSION}-src.tar.gz"
  )
fi
docker_args+=(
  "$BUILD_IMAGE"
  /bin/bash /work/mount_root/container_tcl.sh
)
...
echo "-- Tcl archive ready: ${ARCHIVE_PATH}"
