...
usage() {
  cat <<'EOF'
Usage:
  ./packages/python_dependencies/build.sh --target=<target> [options]
...
Options:
  ...
  --llvm-deps-archive=<tar>     llvm_dependencies archive to use as the base prefix
  --llvm-deps-dir=<dir>         Already extracted llvm_dependencies prefix
...
find_local_llvm_deps_archive() {
  local archive_name="llvm_dependencies-${PACKAGE_TRIPLE}.tar.xz"
  local archive_path=""
...
copy_or_extract_base_prefix() {
  ...
  local dir_path="$3"
  local package_dir="llvm_dependencies-${PACKAGE_TRIPLE}"
  ...
  if [[ -n "$dir_path" ]]; then
    [[ -d "$dir_path" ]] || die "llvm_dependencies directory not found: ${dir_path}"
    cp -a "${dir_path}/." "$output_dir/"
  else
    [[ -f "$archive_path" ]] || die "llvm_dependencies archive not found: ${archive_path}"
    mkdir -p "$tmp_extract"
    ...
    if [[ -d "${tmp_extract}/${package_dir}" ]]; then
      ...
      die "could not find ${package_dir} in archive: ${archive_path}"
    fi
...
validate_base_prefix() {
  ...
  [[ -f "${dir}/README.llvmsdk-deps" ]] || die "missing llvm_dependencies marker: ${dir}/README.llvmsdk-deps"
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --llvm-version)
      ...
      LLVM_VERSION="$1"
      ;;
    --llvm-deps-archive=*|--dependency-archive=*)
      LLVM_DEPS_ARCHIVE="${1#*=}"
      ;;
    --llvm-deps-archive|--dependency-archive)
      opt="$1"
...
resolve_target "$TARGET" "Python dependency target"

if [[ -n "$LLVM_DEPS_ARCHIVE" && -n "$LLVM_DEPS_DIR" ]]; then
  die "--llvm-deps-archive and --llvm-deps-dir are mutually exclusive"
fi

if [[ -z "$PACKAGE_NAME" ]]; then
  PACKAGE_NAME="python_dependencies-${PACKAGE_TRIPLE}"
fi

if [[ -z "$LLVM_DEPS_ARCHIVE" && -z "$LLVM_DEPS_DIR" ]]; then
  LLVM_DEPS_ARCHIVE="$(find_local_llvm_deps_archive)" \
    || die "llvm_dependencies archive not provided and default archive was not found for ${PACKAGE_TRIPLE}"
fi
...
[[ -f "${MOUNT_ROOT}/container_python_dep.sh" ]] || die "missing container script: ${MOUNT_ROOT}/container_python_dep.sh"

make_host_writable "$PACKAGE_ROOT"
mkdir -p "$CACHE_DIR" "$BUILD_DIR" "$OUT_BASE" "$DIST_DIR"

if [[ "$CLEAN" -eq 1 ]]; then
  echo "-- cleaning Python dependencies target: ${TARGET_TRIPLE}"
  rm -rf "$BUILD_DIR" "$OUT_DIR" "$ARCHIVE_PATH"
  mkdir -p "$BUILD_DIR"
fi

copy_or_extract_base_prefix "$OUT_DIR" "$LLVM_DEPS_ARCHIVE" "$LLVM_DEPS_DIR"
...
echo "-- Python dependencies build"
...
if [[ -n "$LLVM_DEPS_ARCHIVE" ]]; then
  echo "-- base dependency archive: ${LLVM_DEPS_ARCHIVE}"
else
  echo "-- base dependency dir: ${LLVM_DEPS_DIR}"
fi
...
docker run --rm \
  ...
  "$BUILD_IMAGE" \
  /bin/bash /work/mount_root/container_python_dep.sh
...
echo "-- Python dependency archive ready: ${ARCHIVE_PATH}"
