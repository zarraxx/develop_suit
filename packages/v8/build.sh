#!/usr/bin/env bash

set -euo pipefail

ROOT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
PROJECT_ROOT="$(cd "${ROOT_DIR}/../.." && pwd)"
SHELL_TOOLS_DIR="${PROJECT_ROOT}/packages/shell_tools"

source "${SHELL_TOOLS_DIR}/var.sh"
source "${SHELL_TOOLS_DIR}/tools.sh"

usage() {
  cat <<'EOF'
Usage:
  ./packages/v8/build.sh --target=<target> [options]

Targets:
  x86_64, x86_64-unknown-linux-gnu
  aarch64, arm64, aarch64-unknown-linux-gnu
  riscv64, riscv64-unknown-linux-gnu
  loongarch64, loongarch64-unknown-linux-gnu
  mingw64, windows, x86_64-w64-windows-gnu

Options:
  --target=<target>             V8 target, see list above
  --arch=<target>               Alias for --target
  --v8-version=<ver>            Official V8 git tag/version (default: 11.6.189.4)
  --llvm-version=<ver>          Bootstrap LLVM toolchain version (default: 18.1.8)
  --llvmsdk-archive=<tar>       Use a prebuilt llvmsdk tarball for target zlib
  --llvmsdk-release-tag=<tag>   GitHub release tag for default llvmsdk download
                                (default: llvmsdk-<llvm-version>)
  --image=<image>               Build image
                                (default: ghcr.io/zarraxx/develop_suit:llvm-with-mingw64-18.1.8)
  --jobs=<n>                    Parallel build jobs inside container (default: 4)
  --package-name=<name>         Override the top-level directory and tarball stem
  --pull                        Pull the selected build image before building
  --clean                       Remove this target's build and output directories first
  -h, --help                    Show this help

Outputs:
  packages/v8/build/dist/v8-<version>-<triple>.tar.xz
EOF
}

TARGET=""
V8_VERSION="11.6.189.4"
LLVM_VERSION="18.1.8"
LLVMSDK_ARCHIVE=""
LLVMSDK_RELEASE_TAG=""
BUILD_IMAGE="$PACKAGES_DEFAULT_BUILD_IMAGE"
JOBS="$PACKAGES_DEFAULT_JOBS"
PACKAGE_NAME=""
PULL=0
CLEAN=0

while [[ $# -gt 0 ]]; do
  case "$1" in
    --target=*|--arch=*) TARGET="${1#*=}" ;;
    --target|--arch)
      opt="$1"
      shift
      [[ $# -gt 0 ]] || die "${opt} requires a value"
      TARGET="$1"
      ;;
    target=*|arch=*) TARGET="${1#*=}" ;;
    --v8-version=*) V8_VERSION="${1#*=}" ;;
    --v8-version)
      shift
      [[ $# -gt 0 ]] || die "--v8-version requires a value"
      V8_VERSION="$1"
      ;;
    --llvm-version=*) LLVM_VERSION="${1#*=}" ;;
    --llvm-version)
      shift
      [[ $# -gt 0 ]] || die "--llvm-version requires a value"
      LLVM_VERSION="$1"
      ;;
    --llvmsdk-archive=*) LLVMSDK_ARCHIVE="${1#*=}" ;;
    --llvmsdk-archive)
      shift
      [[ $# -gt 0 ]] || die "--llvmsdk-archive requires a value"
      LLVMSDK_ARCHIVE="$1"
      ;;
    --llvmsdk-release-tag=*) LLVMSDK_RELEASE_TAG="${1#*=}" ;;
    --llvmsdk-release-tag)
      shift
      [[ $# -gt 0 ]] || die "--llvmsdk-release-tag requires a value"
      LLVMSDK_RELEASE_TAG="$1"
      ;;
    --image=*|--linux-image=*) BUILD_IMAGE="${1#*=}" ;;
    --image|--linux-image)
      opt="$1"
      shift
      [[ $# -gt 0 ]] || die "${opt} requires a value"
      BUILD_IMAGE="$1"
      ;;
    --jobs=*) JOBS="${1#*=}" ;;
    --jobs)
      shift
      [[ $# -gt 0 ]] || die "--jobs requires a value"
      JOBS="$1"
      ;;
    --package-name=*) PACKAGE_NAME="${1#*=}" ;;
    --package-name)
      shift
      [[ $# -gt 0 ]] || die "--package-name requires a value"
      PACKAGE_NAME="$1"
      ;;
    --pull) PULL=1 ;;
    --clean) CLEAN=1 ;;
    -h|--help)
      usage
      exit 0
      ;;
    *) die "unknown argument: $1" ;;
  esac
  shift
done

[[ -n "$TARGET" ]] || die "--target is required"
resolve_target "$TARGET" "V8 target"
case "${TARGET_KIND}:${ARCH}" in
  linux:x86_64|linux:aarch64|linux:riscv64|linux:loongarch64|mingw:x86_64) ;;
  *) die "official GN V8 package currently supports x86_64/aarch64/riscv64/loongarch64 Linux and experimental x86_64 MinGW" ;;
esac

if [[ -z "$PACKAGE_NAME" ]]; then
  PACKAGE_NAME="v8-${V8_VERSION}-${PACKAGE_TRIPLE}"
fi

require_command docker
require_command tar
require_command git

MOUNT_ROOT="${ROOT_DIR}/mount_root"
CACHE_DIR="${PROJECT_ROOT}/cache"
LOCAL_DEPOT_TOOLS_DIR="${ROOT_DIR}/depot_tools"
DEPOT_TOOLS_DIR="${LOCAL_DEPOT_TOOLS_DIR}"
PACKAGE_ROOT="${ROOT_DIR}/build"
BUILD_DIR="${PACKAGE_ROOT}/work/${TARGET_TRIPLE}"
INPUT_DIR="${PACKAGE_ROOT}/inputs"
OUT_BASE="${PACKAGE_ROOT}/out"
OUT_DIR="${OUT_BASE}/${PACKAGE_NAME}"
DIST_DIR="${PACKAGE_ROOT}/dist"
ARCHIVE_PATH="${DIST_DIR}/${PACKAGE_NAME}.tar.xz"
LLVMSDK_ASSET="llvmsdk-${LLVM_VERSION}-${TARGET_TRIPLE}.tar.xz"

if [[ -z "$LLVMSDK_RELEASE_TAG" ]]; then
  LLVMSDK_RELEASE_TAG="llvmsdk-${LLVM_VERSION}"
fi

if [[ -z "$LLVMSDK_ARCHIVE" ]]; then
  if [[ -f "${PROJECT_ROOT}/packages/llvm/build/dist/${LLVMSDK_ASSET}" ]]; then
    LLVMSDK_ARCHIVE="${PROJECT_ROOT}/packages/llvm/build/dist/${LLVMSDK_ASSET}"
  else
    LLVMSDK_ARCHIVE="${INPUT_DIR}/${LLVMSDK_ASSET}"
  fi
fi

[[ -f "${MOUNT_ROOT}/container_v8.sh" ]] || die "missing container script: ${MOUNT_ROOT}/container_v8.sh"
if [[ ! -d "$DEPOT_TOOLS_DIR" ]]; then
  DEPOT_TOOLS_DIR="${CACHE_DIR}/depot_tools"
fi

make_host_writable "$PACKAGE_ROOT"
mkdir -p "$CACHE_DIR" "$BUILD_DIR" "$INPUT_DIR" "$OUT_DIR" "$DIST_DIR"
if [[ ! -d "$DEPOT_TOOLS_DIR" ]]; then
  echo "-- cloning depot_tools: ${DEPOT_TOOLS_DIR}"
  git clone https://chromium.googlesource.com/chromium/tools/depot_tools.git "$DEPOT_TOOLS_DIR"
fi
make_host_writable "$DEPOT_TOOLS_DIR"

if [[ "$CLEAN" -eq 1 ]]; then
  echo "-- cleaning V8 target: ${TARGET_TRIPLE}"
  rm -rf "$BUILD_DIR" "$OUT_DIR" "$ARCHIVE_PATH"
  mkdir -p "$BUILD_DIR" "$OUT_DIR"
fi

if [[ ! -f "$LLVMSDK_ARCHIVE" ]]; then
  require_command curl
  echo "-- downloading llvmsdk archive: ${LLVMSDK_RELEASE_TAG}/${LLVMSDK_ASSET}"
  curl -fL \
    "https://github.com/zarraxx/develop_suit/releases/download/${LLVMSDK_RELEASE_TAG}/${LLVMSDK_ASSET}" \
    -o "$LLVMSDK_ARCHIVE"
fi
[[ -f "$LLVMSDK_ARCHIVE" ]] || die "missing llvmsdk archive: ${LLVMSDK_ARCHIVE}"

if [[ "$PULL" -eq 1 ]]; then
  echo "-- pulling build image: ${BUILD_IMAGE}"
  docker pull --platform linux/amd64 "$BUILD_IMAGE"
fi

echo "-- V8 build"
echo "-- image: ${BUILD_IMAGE}"
echo "-- target kind: ${TARGET_KIND}"
echo "-- target triple: ${TARGET_TRIPLE}"
echo "-- V8 version: ${V8_VERSION}"
echo "-- llvmsdk archive: ${LLVMSDK_ARCHIVE}"
echo "-- package: ${PACKAGE_NAME}"
echo "-- output: ${OUT_DIR}"

docker_args=(
  run --rm
  --platform linux/amd64
  -v "${SHELL_TOOLS_DIR}:/work/shell_tools:ro"
  -v "${MOUNT_ROOT}:/work/mount_root:ro"
  -v "${DEPOT_TOOLS_DIR}:/work/depot_tools"
  -v "${CACHE_DIR}:/work/cache"
  -v "$(cd "$(dirname "$LLVMSDK_ARCHIVE")" && pwd):/work/inputs:ro"
  -v "${BUILD_DIR}:/work/build"
  -v "${OUT_DIR}:/opt/${PACKAGE_NAME}"
  --workdir /work
  -e "ARCH=${ARCH}"
  -e "TARGET_KIND=${TARGET_KIND}"
  -e "TARGET_TRIPLE=${TARGET_TRIPLE}"
  -e "LLVM_VERSION=${LLVM_VERSION}"
  -e "V8_VERSION=${V8_VERSION}"
  -e "JOBS=${JOBS}"
  -e "GCLIENT_JOBS=${GCLIENT_JOBS:-2}"
  -e "SDK_PREFIX=/opt/${PACKAGE_NAME}"
  -e "DEPOT_TOOLS_DIR=/work/depot_tools"
  -e "LLVMSDK_ARCHIVE=/work/inputs/$(basename "$LLVMSDK_ARCHIVE")"
  -e "HOME=/tmp"
  -e "XDG_CACHE_HOME=/work/cache/xdg"
)
docker_args+=(
  "$BUILD_IMAGE"
  /bin/bash /work/mount_root/container_v8.sh
)

docker "${docker_args[@]}"

make_host_writable "$PACKAGE_ROOT"
normalize_package_permissions "$OUT_DIR"

rm -f "$ARCHIVE_PATH"
tar -C "$OUT_BASE" -cJf "$ARCHIVE_PATH" "$PACKAGE_NAME"

echo "-- V8 archive ready: ${ARCHIVE_PATH}"
