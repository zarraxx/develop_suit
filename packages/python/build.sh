...
usage() {
  cat <<'EOF'
Usage:
  ./packages/python/build.sh --target=<target> [options]
...
Options:
  ...
  --python-archive=<tar>             Use a local CPython source archive
...
resolve_target "$TARGET" "Python target"
...
if [[ -z "$PACKAGE_NAME" ]]; then
  PACKAGE_NAME="python-${PYTHON_VERSION}-${PACKAGE_TRIPLE}"
fi
...
[[ -f "${MOUNT_ROOT}/container_python.sh" ]] || die "missing container script: ${MOUNT_ROOT}/container_python.sh"

make_host_writable "$PACKAGE_ROOT"
mkdir -p "$CACHE_DIR" "$BUILD_DIR" "$OUT_BASE" "$DIST_DIR"

if [[ "$CLEAN" -eq 1 ]]; then
  echo "-- cleaning Python target: ${TARGET_TRIPLE}"
...
if [[ -n "$PYTHON_ARCHIVE" ]]; then
  [[ -f "$PYTHON_ARCHIVE" ]] || die "Python source archive not found: ${PYTHON_ARCHIVE}"
fi
...
if [[ -n "$PYTHON_ARCHIVE" ]]; then
  docker_args+=(
    -v "${PYTHON_ARCHIVE}:/work/input/Python-${PYTHON_VERSION}.tar.xz:ro"
    -e "PYTHON_ARCHIVE=/work/input/Python-${PYTHON_VERSION}.tar.xz"
  )
fi
docker_args+=(
  "$BUILD_IMAGE"
  /bin/bash /work/mount_root/container_python.sh
)
...
echo "-- Python archive ready: ${ARCHIVE_PATH}"
