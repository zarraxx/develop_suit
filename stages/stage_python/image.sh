...
Options:
  ...
  --output=<path>           Output Docker archive path
                            (default: <repo>/dist/images/stage-python-image-<arch>.tar)
  --dockerfile=<path>       Override Dockerfile path
                            (default: <repo>/stages/stage_python/Dockerfile)
...
ROOTFS_DIR="${PROJECT_ROOT}/dist/stage_python/${ARCH}"

[[ -d "$ROOTFS_DIR" ]] || die "stage_python rootfs directory does not exist: $ROOTFS_DIR"
...
if [[ ${#TAGS[@]} -eq 0 ]]; then
  TAGS=("stage-python-rootfs:${ARCH}")
fi

if [[ "$PUSH" -eq 0 && -z "$OUTPUT" ]]; then
  OUTPUT="${PROJECT_ROOT}/dist/images/stage-python-image-${ARCH}.tar"
fi

echo "Building stage_python Docker image for arch=${ARCH} platform=${PLATFORM}"
...
build_args=(
  ...
  --platform "${PLATFORM}"
  --build-arg "STAGE_PYTHON_ARCH=${ARCH}"
  --provenance=false
  --sbom=false
)
...
if [[ "$SKIP_TEST" -eq 0 ]]; then
  ...
  docker run --rm -i \
    --platform "${PLATFORM}" \
    -v "${TESTS_DIR}:/opt/stage_python_tests:ro" \
    --entrypoint /bin/sh \
    "${test_tag}" < "${SMOKE_TEST_SCRIPT}"
