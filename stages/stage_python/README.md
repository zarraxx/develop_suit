...
```bash
./stages/stage_python/build.sh --arch=aarch64 --clean --jobs=4
```

或：

```bash
./stages/stage_python/build.sh --arch=x86_64 --clean --jobs=4
```
...
```bash
./stages/stage_python/build.sh --arch=x64 --clean --jobs=4
```
...
```bash
./stages/stage_python/image.sh --arch=x64
```

或：

```bash
./stages/stage_python/image.sh --arch=aarch64
```

默认会在容器里执行 `stages/stage_python/smoke-test.sh`，并挂载 `stages/stage_python/tests/` 里的 Python 测试脚本。
