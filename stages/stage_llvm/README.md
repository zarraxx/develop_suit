...
```bash
./stages/stage_llvm/build.sh --arch=x86_64 --clean --jobs=4
```

比如从当前机器交叉产出一个 `aarch64` host clang：

```bash
./stages/stage_llvm/build.sh --arch=aarch64 --clean --jobs=4
```

构建镜像：

```bash
./stages/stage_llvm/image.sh --arch=x86_64
```

构建完成后可以直接跑冒烟测试，检查 `helloworld.c` / `helloworld.cpp` 是否都能编成 4 个平台的 ELF：

```bash
./stages/stage_llvm/smoke-test.sh dist/stage_llvm/x86_64/opt/llvm-18.1.8
```

也可以指定输出目录，脚本会把 8 个 hello world ELF 写到这里，方便在宿主机继续用 `file` 检查：

```bash
./stages/stage_llvm/smoke-test.sh dist/stage_llvm/x86_64/opt/llvm-18.1.8 /tmp/stage_llvm-smoke-out
