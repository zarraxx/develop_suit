...
usage() {
  cat <<'EOF'
Usage:
  ./stages/stage_tools/image.sh --arch=<arch> [options]
  ./stages/stage_tools/image.sh arch=<arch> [options]
...
Options:
  ...
  --output=<path>           Output Docker archive path
                            (default: <repo>/dist/images/stage-tools-image-<arch>.tar)
  --dockerfile=<path>       Override Dockerfile path
                            (default: <repo>/stages/stage_tools/Dockerfile)
...
TOOLS_ROOTFS_DIR="${ROOT_DIR}/build/out/${ARCH}"

[[ -d "$TOOLS_ROOTFS_DIR" ]] || die "stage_tools output directory does not exist: $TOOLS_ROOTFS_DIR"
...
[[ -d "$TESTS_DIR" ]] || die "tests directory does not exist: $TESTS_DIR"

if [[ ${#TAGS[@]} -eq 0 ]]; then
  TAGS=("stage-tools-rootfs:${ARCH}")
fi

if [[ "$PUSH" -eq 0 && -z "$OUTPUT" ]]; then
  OUTPUT="${PROJECT_ROOT}/dist/images/stage-tools-image-${ARCH}.tar"
fi
...
echo "Building stage_tools Docker image for arch=${ARCH} platform=${PLATFORM}"
echo "Using base image: ${BASE_IMAGE}"
echo "Using stage_tools rootfs overlay: ${TOOLS_ROOTFS_DIR}"
for tag in "${TAGS[@]}"; do
  echo "Using tag: ${tag}"
done
...
build_args=(
  ...
  --platform "${PLATFORM}"
  --build-arg "STAGE_TOOLS_ARCH=${ARCH}"
  --build-arg "STAGE_TOOLS_BASE_IMAGE=${BASE_IMAGE}"
)
...
if [[ "$SKIP_TEST" -eq 0 ]]; then
  ...
  test_tag="${TAGS[0]}"
  smoke_output_dir="${PROJECT_ROOT}/dist/stage_tools-smoke/${ARCH}"
  ...
  docker run --rm -i \
    --platform "${PLATFORM}" \
    -v "${ROOT_DIR}:/opt/stage_tools:ro" \
    -v "${smoke_output_dir}:/opt/stage_tools_out" \
    --entrypoint /bin/sh \
    "${test_tag}" \
    /opt/stage_tools/smoke-test.sh /opt/stage_tools/tests /opt/stage_tools_out
