...
比如 
把 stages/stage_tools/mount_root 挂入容器
...
```bash
./stages/stage_tools/build.sh --arch=aarch64 --jobs=4
```

构建并测试镜像：

```bash
./stages/stage_tools/image.sh --arch=aarch64
```

发布 4 个架构镜像时分别执行：

```bash
./stages/stage_tools/build.sh --arch=x86_64 --jobs=4
./stages/stage_tools/image.sh --arch=x86_64 --push --tag=ghcr.io/zarraxx/develop_suit:tmp-stage-tools-x86_64

./stages/stage_tools/build.sh --arch=aarch64 --jobs=4
./stages/stage_tools/image.sh --arch=aarch64 --push --tag=ghcr.io/zarraxx/develop_suit:tmp-stage-tools-aarch64

./stages/stage_tools/build.sh --arch=riscv64 --jobs=4
./stages/stage_tools/image.sh --arch=riscv64 --push --tag=ghcr.io/zarraxx/develop_suit:tmp-stage-tools-riscv64

./stages/stage_tools/build.sh --arch=loongarch64 --jobs=4
./stages/stage_tools/image.sh --arch=loongarch64 --push --tag=ghcr.io/zarraxx/develop_suit:tmp-stage-tools-loongarch64
```
