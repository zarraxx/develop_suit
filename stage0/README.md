...
```bash
./stage0/build.sh --arch=x86_64
./stage0/build.sh --arch=aarch64
./stage0/build.sh --arch=riscv64
./stage0/build.sh --arch=loongarch64
```

默认行为：

1. 在 `stage0/build/<arch>/` 下创建对应架构构建目录
...
```bash
./stage0/build.sh --arch=x86_64 --cmake-arg=-DSTAGE0_DOWNLOAD_MISSING=OFF
```
...
```bash
stage0/busybox-stage0.configfrag
```
...
```bash
./stage0/build.sh --arch=x86_64 --config-fragment=/abs/path/my-busybox.configfrag
```
